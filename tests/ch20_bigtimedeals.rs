...
fn format_as_date(time: time::Date) -> String {
    static FORMAT: once_cell::race::OnceBox<Vec<time::format_description::FormatItem<'static>>> =
...
mod tests {
    ...
    async fn localstack_only_test() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        for i in 0..100 {
            let app = app.clone();
            handles
                .spawn(async move { app.create_user(UserName::from(format!("mtest_{i}"))).await });
        }
        while handles
            .join_next()
            .await
